...
Avaa QGISin Python konsoli.

![](img/kappale_12/image1.png)
**Aineisto:** ```valtiot.gpkg```

1. Lisää ```valtiot.gpkg```- aineisto QGISiin ja valitse taso. Kokeile ensin peruskomentoja konsolissa:
...
2. Käy for- silmukassa läpi "valtiot"- tason 10 ensimmäistä kohdetta ja tulosta kohteen nimi (ADMIN- kenttä).
...
3. Tarkastele seuraavaa skriptiä. Pohdi mitä skripti tekee sen eri vaiheissa?
...
countries_layer = QgsProject.instance().mapLayersByName('hallintoalueet')[0]

new_layer = QgsVectorLayer('Polygon?crs=epsg:3067', 'suomi_vyohyke', 'memory')
...
finland_feature = countries_layer.getFeature(1)

finland_geometry = finland_feature.geometry()

buffered_geometry = finland_geometry.buffer(5 * 1000, 4)
